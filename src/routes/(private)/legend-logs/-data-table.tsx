  ...
  const logsByDate = useMemo(() => {
    ...
    sortedDates.forEach((dateKey) => {
      ...
      logs.forEach((log) => {
        ...
        // GAIN e LOSS: somar TODOS os ataques e defesas
        // trophiesResult é a mudança de troféus (positivo = ganhou, negativo = perdeu)
        const trophiesChange = log.trophiesResult;
        
        if (log.type === "ATTACK" || log.type === "DEFENSE") {
          if (trophiesChange > 0) {
            // Ganhou troféus - SOMAR TODOS
            playerLog.gain += trophiesChange;
            playerLog.gainCount++;
          } else if (trophiesChange < 0) {
            // Perdeu troféus - SOMAR TODOS (valor absoluto)
            playerLog.loss += Math.abs(trophiesChange);
            playerLog.lossCount++;
          }
          // Se trophiesResult === 0, não conta
        }
      });
      ...
      playerLogs.forEach((playerLog) => {
        ...
        if (player) {
          // O FINAL é o troféu atual menos o net change deste dia (pois os troféus atuais incluem este dia)
          const netChange = playerLog.gain - playerLog.loss;
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                          {row.original.logs
                            ...
                            .map((log: LegendLog) => {
                              const isPositive = log.trophiesResult >= 0;
                              const count = Math.abs(log.diff);
                              return (
                                <div
                                  key={log.id}
                                ...
                                >
                                  <div className="flex items-center justify-between">
                                    ...
                                    >
                                      {formatWithSuperscript(log.trophiesResult, count)}
                                    </span>
                                  </div>
